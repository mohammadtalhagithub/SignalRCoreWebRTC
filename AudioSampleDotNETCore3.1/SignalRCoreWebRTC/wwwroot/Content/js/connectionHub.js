...
attachMediaStream = (e) => {
    ...
    if (partnerVideo.srcObject !== e.stream) {
...
const receivedCandidateSignal = (connection, partnerClientId, candidate) => {
    ...
    const partnerUser = globalUserList.find(user => user.connectionId === partnerClientId);

    if (partnerUser) {
        console.log(`Partner Found and receivedCandidateSignal : Username = ${partnerUser.username}, ConnectionId = ${partnerUser.connectionId}`);
    }
...
const receivedSdpSignal = (connection, partnerClientId, sdp) => {
    ...
    if (partnerUser) {
        console.log(`Partner Found and Set Remote description : Username = ${partnerUser.username}, ConnectionId = ${partnerUser.connectionId}`);
    }
        ...
        if (connection.remoteDescription.type == "offer") {
            ...
            connection.createAnswer().then((desc) => {
                ...
                connection.setLocalDescription(desc, () => {
                    //console.log('WebRTC: set Local Description...' + desc);
                    //console.log('connection.localDescription: ', connection.localDescription);
                    //setTimeout(() => {
                    const partnerUser = globalUserList.find(user => user.connectionId === partnerClientId);

                    if (partnerUser) {
                        console.log(`Partner Found and Set local description : Username = ${partnerUser.username}, ConnectionId = ${partnerUser.connectionId}`);
                    }
                    console.log("setRemoteDescription == " + partnerClientId);
...
const newSignal = (partnerClientId, data) => {
    ...
    if (signal.sdp) {
        ...
        console.log('WebRTC:  AAAAAAAAAAAAAAAAAAAAAAAAAA candidate signal');
        ...
        //if (partnerUser) {
        //    console.log(`Partner Found and addIceCandidate : Username = ${partnerUser.username}, ConnectionId = ${partnerUser.connectionId}`);
        //}
        //connection.addIceCandidate(null, () => console.log("WebRTC: added null candidate successfully"), () => console.log("WebRTC: cannot add null candidate"));
    }
}
...
const initiateOffer = (partnerClientId, stream) => {
    ...
    connection.createOffer().then(offer => {
        ...
        connection.setLocalDescription(offer).then(() => {
            ...
            setTimeout(() => {
                console.log("setLocalDescription == " + partnerClientId);
                sendHubSignal(JSON.stringify({ "sdp": connection.localDescription }), partnerClientId);
            }, 1000);
...
const callbackUserMediaSuccess = (stream) => {
    ...
    //console.log("call under callbackUserMediaSuccess 3");
    localStream = stream;
    const localVideo = document.querySelector('.video.mine');
    if (localVideo) {
        localVideo.srcObject = stream; // Attach stream to the local video element
    }
...
const initializeUserMedia = () => {
    ...
    //console.log("call under initializeUserMedia 2");
...
const callbackIceCandidate = (evt, connection, partnerClientId) => {
    //console.log("WebRTC: callbackIceCandidate 1");
    //console.log("evt.candidate: ", evt.candidate);
    if (evt.candidate) {// Found a new candidate
        console.log("if  callbackIceCandidate");
        //console.log("WebRTC: in callbackIceCandidate if ");
        //console.log('WebRTC: new ICE candidate');
        //console.log("evt.candidate: ", evt.candidate);
        sendHubSignal(JSON.stringify({ "candidate": evt.candidate }), partnerClientId);
    }
    else {
        console.log("else callbackIceCandidate");
        //console.log("WebRTC: in callbackIceCandidate else ");
...
const initializeConnection = (partnerClientId) => {
    //console.log('WebRTC: Initializing connection...' + partnerClientId);
...
sendHubSignal = (candidate, partnerClientId) => {
    //console.log('candidate', candidate);
    //console.log("WebRTC:sendHubSignal ");
...
wsconn.on('updateUserList', (userList) => {
    ...
    $.each(userList, function (index) {
        var userIcon = '', status = '';
        //console.log(`User ${index + 1}: Username = ${userList[index].username}, ConnectionId = ${userList[index].connectionId}`);         
...
wsconn.on('incomingCall', (callingUser) => {
    ...
    alertify.confirm(callingUser.username + ' is calling.  Do you want to chat?', function (e) {
        if (e) {
            //console.log("I want to chat =" + callingUser.username);
            // I want to chat
            ...
            //console.log("Go away, I don't want to chat with you =" + callingUser.username);
...
const setUsername = (username) => {
    ...
    //console.log("call before initializeUserMedia 1");
    initializeUserMedia();
};
